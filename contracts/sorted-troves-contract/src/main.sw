...
impl SortedTroves for Contract {
    ...
    fn set_params(
        ...
        storage.borrower_operations_contract.write(borrower_operations_contract);
...
fn require_is_protocol_manager() {
    ...
    require(Identity::ContractId(storage.protocol_manager.read()) == sender, "ST: Not PM");
}
...
fn internal_find_insert_position(
    ...
    if (prev_id == null_identity_address()
        && next_id == null_identity_address())
    {
        return internal_descend_list(nicr, internal_get_head(asset), asset, trove_manager_contract);
    } else if (prev_id == null_identity_address()) {
...
fn internal_insert(
    ...
    if (prev_id == null_identity_address() && next_id == null_identity_address()) {
        ...
        edit_node_neighbors(internal_get_head(asset), Option::Some(id), Option::None, asset);
        ...
        edit_node_neighbors(internal_get_tail(asset), Option::None, Option::Some(id), asset);
