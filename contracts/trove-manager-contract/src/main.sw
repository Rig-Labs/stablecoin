...
impl TroveManager for Contract {
    ...
    fn has_pending_rewards(id: Identity) -> bool
 {
    ...
    #[storage(read)]
    fn get_current_icr(id: Identity, price: u64
) -> u64 {
    ...
    #[storage(read)]
    fn get_entire_debt_and_coll(id: Identity) -> (
u64, u64, u64, u64) {
    ...
    fn get_trove_stake(id: Identity) -> u64
 {
    ...
    #[storage(read, write)]
    fn set_trove_status(id: Identity, status:
 Status) {
...
trove) => {
    ...
    #[storage(read, write)]
    fn increase_trove_coll(id: Identity, coll:
 u64) -> u64 {
        require_caller_is_borrow_operations_contract();
...
(id: Identity, value:
 u64) -> u64 {
    ...
    #[storage(read, write)]
    fn add_trove_owner_to_array
(id: Identity) ->
 u64 {
        require_caller_is_borrow_operations_contract();
        storage.trove_owners.push(id);
        let indx = storage.trove_owners.
len() - 1;
...
;
    ...
    #[storage(read)]
    fn get_pending_usdf_rewards(
address: Identity) -> u64
 {
...
#[storage
(read, write)]
fn internal_close_trove(id: Identity, close_status:
 Status) {
...
fn internal_remove_trove_owner(_borrower: Identity, _trove_array_owner_length: u64) {
    ...
    let indx_last = length
 - 1;
    require(index <= indx_last, "TM: Trove does not exist");
    let address_to_move = storage.trove_owners.get
(indx_last).unwrap().read();
...
Identity,
    ...
    let oracle = abi(MockOracle, storage
.oracle_contract.read().into());
    let asset_contract_cache = storage.asset_contract.read();
    vars.price = oracle.get_price();
    let stability_pool = abi(StabilityPool
, storage.stability_pool_contract.read().into());
    ...
    require(
totals.total_debt_in_sequence > 0, "TM: No debt to liquidate");
    ...
    if (totals.total_coll_surplus > 0) {
        active_pool.send_asset(Identity::ContractId(storage.
coll_surplus_pool_contract.read()), totals.total_coll_surplus, asset_contract_cache);
    }
...
#[storage(read, write)]
fn internal_increase_trove_coll(id: Identity, coll:
 u64) -> u64 {
    let mut trove = storage.troves.get(id).read();
    trove.coll += coll;
    storage.troves.insert(id, trove);
...
#[storage(read, write)]
fn internal_increase_trove_debt
(id: Identity, debt:
 u64) -> u64 {
    let mut trove = storage.troves.get(id).read();
    trove.debt += debt;
...
#[storage(read, write)]
fn internal_decrease_trove_coll
(id: Identity, coll:
 u64) -> u64 {
    let mut trove = storage.troves.get(id).read();
    trove.coll -= coll;
...
#[storage(read, write)]
fn internal_decrease_trove_debt
(id: Identity, debt:
 u64) -> u64 {
    let mut trove = storage.troves.get(id).read();
    trove.debt -= debt;
...
(i).unwrap();
        ...
        if vars.icr < MCR {
            ...
            single_liquidation = get_offset_and_redistribution_vals(position.e
ntire_trove_coll, position.entire_trove_debt, usdf_in_stability_pool, price);
            internal_apply_liquidation(
vars.borrower, single_liquidation, upper_partial_hint, lower_partial_hint);
            vars.remaining_usdf_in_stability_pool -= single_liquidation.
debt_to_offset;
...
#[storage(read)]
fn internal_get_current_icr(borrower: Identity, price: u64
) -> u64 {
...
LiquidationValues,
    ...
    if (liquidation_values.is_partial_liquidation) {
        let mut trove = storage.troves.get
(borrower).read();
        ...
        let new_ncr = fm_compute_nominal_cr(trove.coll,
 trove.debt);
        let sorted_troves_contract =
 abi(SortedTroves, storage.sorted_troves_contract.read().into());
...
#[storage(read, write)]
fn internal_redistribute_debt_and_coll(debt: u64, coll:
 u64) {
    let asset_contract_cache = storage.asset_contract.read();
    if (debt == 0) {
        return;
    }
    let asset_numerator: U128 = U128::from_u64(coll) * U128::from_u64(DECIMAL_PRECISION) + U128::from_u64
(storage.last_asset_error_redistribution.read());
    ...
    storage.last_asset_error_redistribution.write((asset_numerator - (asset_reward_per_unit_staked * U128::from_u64(storage.
total_stakes.read()))).as_u64().unwrap());
    ...
    storage.l_asset.write(storage.l_asset.read() + asset_reward_per_unit_staked.
as_u64().unwrap());
...
#[storage(read)]
fn internal_get_pending_usdf_reward(address:
 Identity) -> u64
 {
...
#[storage(read)]
fn internal_has_pending_rewards(address:
 Identity) -> bool
 {
    ...
    return (storage.reward_snapshots.get(address).read().asset
 < storage.l_asset.read());
}
#[storage(read)]
...
#[storage
(read, write)]
fn internal_redeem_close_trove(borrower:
 Identity, usdf_amount: 
u64, asset_amount: u64) {
...
());
    ...
    usdf_contract.burn {
        coins: usdf_amount,
        asset_id: storage.usdf_contract.read().
value,
    }();
