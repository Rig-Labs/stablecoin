...
storage {
 ...
 ContractId = ContractId::from(ZERO_B256),
...
write(fpt_staking);
    ...
    fn register_asset(
        ...
        active_pool.
add_asset(asset_address, Identity::ContractId(trove_manager));
...
Identity,
        ...
        let sorted_troves = abi(SortedTroves, storage.sorted_troves_contract
.read().value);
        let active_pool = abi(ActivePool, storage.active_pool_contract.read().value);
        ...
        let (mut current_borrower, mut index
) = find_min_borrower(assets_info.current_borrowers, assets_info.current_crs);
        let mut remaining_itterations =
 max_itterations;
        while (current_borrower != null_identity_address() && remaining_usdf > 0 && remaining_itterations > 0)
 {
            ...
            let mut totals = assets_info.
redemption_totals.get(index).unwrap();
            ...
            let single_redemption
 = trove_manager_contract.redeem_collateral_from_trove(current_borrower, remaining_usdf, price, partial_redemption_hint, upper_partial_hint, lower_partial_hint);
            ...
            if (next_user_to_check != null_identity_address()) {
                next_cr = trove_manager_contract.get_current_icr(next_user_to_check,
 price);
            }
            assets_info.current_crs.set(index, next_cr);
            assets_info.current_borrowers.set(index, next_user_to_check);
            assets_info.
redemption_totals.set(index, totals);
...
.1;
        ...
        while (ind < assets_info.assets.len(
            ...
            let mut totals = assets_info.
redemption_totals.get(ind).unwrap();
            ...
            totals.asset_fee = fm_compute_redemption_fee(
totals.total_asset_drawn);
...
msg_sender().unwrap(), totals.asset_to_send_to_redeemer, contracts_cache.asset_address);
            ind += 1;
        }
...
fn require_valid_usdf_id() {
    require(msg_asset_id() == get_default_asset_id(storage.u
sdf_token_contract.read()), "PM: Invalid asset being transfered");
}
#[storage(read)]
fn get_all_assets_info() -> AssetInfo {
    ...
    let mut ind = 0;
    while (ind < length) {
        assets.push(storage.assets.get
(ind).unwrap(
).read());
    ...
    let mut i = 0;
    while (i < length) {
        let oracle = abi(MockOracle, asset_contracts.
get(i).unwrap().oracle.into());
...
()) {
            current_cr = trove_manager.get_current_icr(current_borrower,
 price);
        }
        prices.push(price);
        system_debt.push(trove_manager.get_entire_system_debt());
        redemption_totals.push(
RedemptionTotals::default());
...
fn find_min_borrower(current_borrowers: Vec<Identity>, current_crs: Vec<u64>) -> (Identity, u64) {
    let mut min_borrower = current_borrowers
.get(0).unwrap();
