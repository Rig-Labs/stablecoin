...
pub fn
 calculate_redeemable_amount(current_time: u64, vesting_schedule: VestingSchedule) -> u64 {
 ...
 >= vesting_schedule.end_timestamp {
    ...
    let mut
 amount_redeemable = vesting_schedule.cliff_amount;
    let total_minus_cliff
 = vesting_schedule.total_amount - vesting_schedule.cliff_amount;
    ...
    let fraction_amount_claimable
 = fm_multiply_ratio(total_minus_cliff, time_elapsed, total_vesting_duration);
    amount_redeemable +=
 fraction_amount_claimable - vesting_schedule.claimed_amount;
    return amount_redeemable
;
}
pub fn
 is_valid_vesting_schedule(vesting_schedule: VestingSchedule) -> bool {
    if vesting_schedule.cliff_timestamp >= vesting_schedule.end_timestamp
    {
        return false;
    }
    if vesting_schedule
.cliff_amount > vesting_schedule.total_amount
    {
        return false;
    }
    if vesting_schedule
...
fn test_redeemable_calculations() {
    ...
    // Before cliff
    assert(
calculate_redeemable_amount(0, vesting_schedule) == 0);
    // At cliff
 ...
 VestingSchedule {
    ...
    assert(
calculate_redeemable_amount(150, vesting_schedule) == 450);
