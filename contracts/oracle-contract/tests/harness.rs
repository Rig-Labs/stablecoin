...
use test_utils::{
    ...
    interfaces::{
        oracle::{oracle_abi, Oracle, PythConfig, RedstoneConfig, StorkConfig, ORACLE_TIMEOUT},
        pyth_oracle::{
    ...
    setup::common::{
        deploy_mock_pyth_oracle, deploy_mock_redstone_oracle, deploy_mock_stork_oracle,
        deploy_oracle,
    },
};
...
async fn setup(
    ...
    if initialize_pyth {
        ...
        let _ = oracle_abi::set_pyth_config(
            &oracle,
            &pyth_contract,
            PythConfig {
...
mod tests {
    ...
    mod pyth_only_and_with_redstone {
        ...
        async fn returns_updated_pyth_price_without_updating_last_price_when_only_pyth() {
            ...
            let last_price = oracle_abi::get_last_good_price(&oracle)
                .await
                .unwrap()
                .value
                .price;
        ...
        async fn falls_back_to_last_price_when_pyth_stale_and_redstone_stale() {
            ...
            let (oracle, _, pyth, redstone) = setup(
                REDSTONE_PRECISION,
                DEFAULT_FUEL_VM_DECIMALS,
                false,
                true,
    ...
    mod pyth_timeout {
        ...
        async fn uses_redstone_when_pyth_is_stale() {
            ...
            let (oracle, _, pyth, redstone_wrapped) = setup(
                REDSTONE_PRECISION,
                DEFAULT_FUEL_VM_DECIMALS,
                false,
                true,
        ...
        async fn uses_redstone_when_pyth_stale_and_stork_stale() {
            ...
            let (oracle, stork, pyth, redstone_wrapped) = setup(
                REDSTONE_PRECISION,
                DEFAULT_FUEL_VM_DECIMALS,
                true,
                true,
    ...
    mod confidence_check {
        ...
        async fn ignores_confidence_when_within_threshold_if_pyth_fresh() {
            ...
            let result = oracle_abi::get_price(&oracle).await;
    ...
    mod fuel_vm_decimals {
        ...
        async fn adjusts_price_for_fuel_vm_decimals_8() {
            ...
            let (oracle, _, pyth, _) =
                setup(REDSTONE_PRECISION, fuel_vm_decimals, false, true, true).await;
    ...
    mod pyth_exponent_changes {
        ...
        async fn pyth_price_adjustment_is_consistent_across_exponents() {
            ...
            .await;
            let price_exp_9 = oracle_abi::get_price(&oracle).await;
            ...
            .await;
            let price_exp_6 = oracle_abi::get_price(&oracle).await;
            ...
            .await;
            let price_exp_12 = oracle_abi::get_price(&oracle).await;
